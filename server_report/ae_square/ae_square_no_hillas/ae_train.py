...
def reconstruction_error(original, reconstructed, criterion):
    ...
    return loss.mean(dim=(1,2,3)).cpu().numpy()
...
def main(data_path_train, data_path_test_protons, data_path_test_gammas, model_save_dir, epochs=10, batch_size=64, max_events=None, max_events_test=70000):
    ...
    file_list = glob.glob(os.path.join(data_path_train, "*.h5"))
    subarray = SubarrayDescription.read(file_list[0])
    image_mapper = ImageMapper.from_name("BilinearMapper", geometry=geo, subarray=subarray)


    print(f"Loading dataset from: {data_path_train}")
    start_time = time.time()
    dataset = TelescopeDataset(data_path_train, image_mapper, max_events=max_events)
    end_time = time.time()
    ...
    # Protons test
    dataset_protons = TelescopeDataset(data_path_test_protons, image_mapper, max_events=max_events_test)
    ...
    # Gammas test
    dataset_gammas = TelescopeDataset(data_path_test_gammas, image_mapper, max_events=max_events_test)
