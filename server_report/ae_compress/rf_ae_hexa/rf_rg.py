...
def hillas_to_rf_dataframe(hillas_list, true_energies, leakage_2, slopes):
    data = {
        "log_intensity": [],
        "width": [],
        "length": [],
        ...
        "y": [],
        "timing_slope": [],
        "true_energy": np.log10(true_energies)
    }
...
def get_data(path, ae, max_events=None):
    images_list, masks_list, peaks_list, true_energy_list = load_data(path, max_events)
    ...
    for idx in range(len(images_list)):
        ...
        true_energies.append(true_energy_list[idx])


    df = hillas_to_rf_dataframe(hillas_params, true_energies, leakage_2, timing_slops)
    df_improved = hillas_to_rf_dataframe(hillas_improved_params, true_energies, leakage_2_improved, timing_slops_improved)
...
def get_dataset(df_gamma):

    X = df_gamma.drop(columns=["true_energy"])
    y_energy = df_gamma["true_energy"]
...
def main(data_path_train_protons, data_path_train_gammas, data_path_test_protons, data_path_test_gammas, ae_model_path, model_save_dir, max_events=None, max_events_test=70000):
    ...
    df_gamma, df_improved_gamma = get_data(data_path_train_gammas, ae, max_events=max_events)
    df_gamma_test, df_improved_gamma_test = get_data(data_path_test_gammas, ae, max_events=max_events_test)

    
    print("Creating datasets...", flush=True)
    X, y = get_dataset(df_gamma)
    X_improved, y_improved = get_dataset(df_improved_gamma)
    X_test, y_test = get_dataset(df_gamma_test)
    X_improved_test, y_improved_test = get_dataset(df_improved_gamma_test)

    # Train regressors
    print("Training Random Forest Regressor...", flush=True)
    rfr = get_regressor()
    rfr.fit(X, y)
    print("Training Random Forest Regressor with improved data...", flush=True)
    rfr_improved = get_regressor()
    rfr_improved.fit(X_improved, y_improved)
    print("Testing Random Forest Regressor...", flush=True)
    rfr_score = rfr.score(X_test, y_test)
    print(f"Test R^2 (original data): {rfr_score:.4f}", flush=True)
    rfr_improved_score = rfr_improved.score(X_improved_test, y_improved_test)
    print(f"Test R^2 (improved data): {rfr_improved_score:.4f}", flush=True)

    # Save regressors
    rfr_path = os.path.join(model_save_dir, "rf_regressor.pkl")
    rfr_improved_path = os.path.join(model_save_dir, "rf_regressor_improved.pkl")
    joblib.dump(rfr, rfr_path)
    print(f"Saved Random Forest Regressor to {rfr_path}", flush=True)
    joblib.dump(rfr_improved, rfr_improved_path)
    print(f"Saved Random Forest Regressor with improved data to {rfr_improved_path}", flush=True)

    # Save results to CSV (predicted_probability, predicted energy, true energy, event type)
    results_df = pd.DataFrame({
        "predicted_energy_original": rfr.predict(X_test),
        "predicted_energy_improved": rfr_improved.predict(X_improved_test),
        "true_energy": y_test,
    })
    results_csv_path = os.path.join(model_save_dir, "rf_results_rg.csv")
