...
class TelescopeDataset(Dataset):
    ...
    def _build_dataset(self, images, masks, peaks):
        for image, msk, pk in zip(images, masks, peaks):
            ...
            valid_edges = (mask_tensor[self.edge_index[0]] == 1) & (mask_tensor[self.edge_index[1]] == 1)
            filtered_edge_index = self.edge_index[:, valid_edges.squeeze()]
...
class AE(nn.Module):
    def __init__(self, in_channels=1, hidden_channels=8, latent_dim=2):
        super().__init__()
        # Encodeur GCN
