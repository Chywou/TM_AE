...
class TelescopeDataset(Dataset):
    ...
    def _build_dataset(self, images, masks, peaks):
        for image, msk, pk in zip(images, masks, peaks):
            ...
            self.originals.append([image, pk])

            new_mask = clean_image_improvement(pk, msk, neighbors, max_diff=5)

            # Preprocess the image
            pk = pk * new_mask
            min_peak = np.min(pk[pk.astype(bool)]) - 4

            pk = np.clip(pk, min_peak, 196)
            pk = (pk - min_peak) / (np.max(pk) - min_peak + 1e-6)

            x = torch.from_numpy(pk).float().unsqueeze(1)
