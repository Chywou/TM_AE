...
class TelescopeDataset(Dataset):
    ...
    def _build_dataset(self, images, masks, peaks):
        for image, msk, pk in zip(images, masks, peaks):
            ...
            mask_tensor = torch.from_numpy(new_mask).float().unsqueeze(1)
...
class AE(nn.Module):
    def __init__(self, in_channels=1, hidden_channels=8, latent_channels=1, pool_ratio=0.2):
        super().__init__()

        self.conv = GCNConv(in_channels, hidden_channels)
        self.pool = TopKPooling(hidden_channels, ratio=pool_ratio)
        self.conv_latent = GCNConv(hidden_channels, latent_channels)
    ...
    def encode(self, x, edge_index, batch):
        N0 = x.size(0)
        x = F.leaky_relu(self.conv(x, edge_index), 0.1)

        x_pool, edge_index_p, _, batch_p, perm, _ = self.pool(x, edge_index, batch=batch)

        z = self.conv_latent(x_pool, edge_index_p)

        return z, perm, N0

    def decode(self, z, perm, N0):
        x_dec = self.decoder(z)

        x_up = torch.zeros(N0, x_dec.size(1), device=x_dec.device)
        x_up[perm] = x_dec

        return x_up

    def forward(self, data):
        z, perm, N0 = self.encode(data.x, data.edge_index, data.batch)
        return self.decode(z, perm, N0)
...
def reconstruct_dataset(model, dataset, batch_size=16):
    ...
    with torch.no_grad():
        for batch in dataloader:   
            ...
            masks = batch.mask.view(batch_size_actual, num_nodes, -1).cpu()
    ...
    return originals, normalized, reconstructed, masks_list

def reconstruction_error(original, reconstructed, masks, criterion):
    print(f"Shape original: {original.shape}, Shape reconstructed: {reconstructed.shape}, Shape masks: {masks.shape}")
    loss = criterion(reconstructed, original)
    masked_loss = loss * masks
    per_img_loss = masked_loss.sum(dim=(1, 2)) / masks.sum(dim=(1, 2))
